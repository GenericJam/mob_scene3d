defmodule S3dSpike.HomeScreen do
  @moduledoc "Landing screen for S3dSpike."
  use Mob.Screen
  ...
  def render(assigns) do
    ...
    <Scroll background={:background}>
      <Column background={:background} padding={:space_lg}>
        ...
        <Spacer size={16} />
        <Text text="S3dSpike" text_size={:xl} text_color={:on_surface} padding={:space_sm} />
        ...
        <Spacer size={12} />
        {nav_button("Pawn (scoped material)", :open_pawn)}
  ...
  def handle_info({:tap, :open_text}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, S3dSpike.TextScreen)}
  end

  def handle_info({:tap, :open_list}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, S3dSpike.ListScreen)}
  end
  ...
  def handle_info({:tap, :open_pawn}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, S3dSpike.PawnScreen)}
  end

  def handle_info({:tap, :open_scene3d}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, S3dSpike.Scene3dScreen)}
  end

  def handle_info({:tap, :open_webview}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, S3dSpike.WebViewScreen)}
  end

  def handle_info({:tap, :open_audio}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, S3dSpike.AudioScreen)}
  end

  def handle_info({:tap, :open_storage}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, S3dSpike.StorageScreen)}
  end
