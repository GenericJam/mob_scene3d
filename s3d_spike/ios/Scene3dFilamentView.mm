...
- (void)setupFilament {
    ...
    _skybox = Skybox::Builder().color({0.035f, 0.04f, 0.07f, 1.0f}).build(*_engine);
...
- (void)loadGlb {
    ...
    _asset = _assetLoader->createAsset(
        static_cast<const uint8_t *>(data.bytes), (uint32_t)data.length);
    if (_asset == nullptr) {
    ...
    _baseTransform = mat4f::scaling(float3{scaleFactor}) * mat4f::translation(-center);
    NSLog(@"[scene3d] bounds min=(%f,%f,%f) max=(%f,%f,%f) center=(%f,%f,%f) scale=%f entities=%zu",
          bounds.min.x, bounds.min.y, bounds.min.z, bounds.max.x, bounds.max.y, bounds.max.z,
          center.x, center.y, center.z, scaleFactor, _asset->getEntityCount());
...
- (void)layoutSubviews {
    ...
    _view->setViewport(Viewport(0, 0, (uint32_t)size.width, (uint32_t)size.height));
...
- (void)renderFrame {
    ...
    if (_asset != nullptr) {
        float angle = (float)((CACurrentMediaTime() - _startTime) * (45.0 * M_PI / 180.0));
        TransformManager &tm = _engine->getTransformManager();
