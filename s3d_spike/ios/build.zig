// build.zig — Native build orchestration for S3dSpike (iOS sim).
//
...
pub fn build(b: *std.Build) void {
    ...
    // Spike (mob_scene3d-b9g): the project bridging header wraps mob's and
    // adds the Filament view's ObjC interface; mob's header resolves via
    // the -Xcc -I clang-importer path below.
    ...
    // --- Filament spike (mob_scene3d-b9g): ObjC++ renderer ---------------------
    // Apple clang via xcrun (same rationale as the ObjC steps above); C++17
...
const LinkOptions = struct {
    ...
    // Filament spike (mob_scene3d-b9g): dir holding the vendored Filament
...
fn addLink(b: *std.Build, step: *std.Build.Step, opts: LinkOptions) void {
    ...
    // Filament spike (mob_scene3d-b9g): static archives from the release
    ...
    // symbols; Filament's is listed after it and only fills gaps.
